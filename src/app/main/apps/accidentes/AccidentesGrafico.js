...
function AccidentesGrafico({
  ...
  const handleTipoGrafico = (e) => {
    ...
    if (variableEstudio && variableEstudio !== "") {
      ...
      series = [
        ...new Set(
          geojson_data.map((obj) => obj.properties[variableEstudio.column])
        ),
      ];
      columns = [];
      series.map((s) => {
        let serie_count = geojson_data.filter((f) => {
          return f.properties[variableEstudio.column] == s;
        }).length;
        if (s !== null) {
  ...
  return (
    ...
    >
      ...
      >
        <MenuItem value={"bar"}>Columnas</MenuItem>
        <MenuItem value={"pie"}>Tarta</MenuItem>
      </Select>
    </Stack>
  );
}
